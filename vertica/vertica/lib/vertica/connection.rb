...
class Vertica::Connection
  ...
  def interrupt
    raise Vertica::Error::InterruptImpossible, "Session cannot be interrupted because the session ID is not known!" if session_id.nil?
    conn = self.class.new(options.merge(skip_initialize: true))
