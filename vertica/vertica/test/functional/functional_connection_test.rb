...
class FunctionalConnectionTest < Minitest::Test
  ...
  def test_interrupt_on_non_interruptable_connection
    connection = Vertica::Connection.new(TEST_CONNECTION_HASH.merge(interruptable: false))
  ...
  def test_initialize_connection_with_search_path
    connection = Vertica::Connection.new(TEST_CONNECTION_HASH.merge(search_path: 'public'))
    assert_equal "public, v_catalog, v_monitor, v_internal", connection_setting(connection, 'search_path')

    connection = Vertica::Connection.new(TEST_CONNECTION_HASH.merge(search_path: ['v_catalog', 'v_monitor', 'v_internal']))
  ...
  def test_initialize_connection_with_role
    ...
    connection = Vertica::Connection.new(TEST_CONNECTION_HASH.merge(role: :none))
    enabled_roles = connection.query('show enabled roles').fetch(0, 'setting').split(', ')
    assert_equal [], enabled_roles

    connection = Vertica::Connection.new(TEST_CONNECTION_HASH.merge(role: :all))
    enabled_roles = connection.query('show enabled roles').fetch(0, 'setting').split(', ')
    assert_equal available_roles, enabled_roles
  end

  def test_initialize_connection_with_timezone
    connection = Vertica::Connection.new(TEST_CONNECTION_HASH.merge(timezone: 'America/Toronto'))
